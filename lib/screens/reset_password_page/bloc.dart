...
import 'package:vpn/widgets/show_custom_dialog.dart';
...
class ResetPasswordBloc extends BlocBaseWithState<ScreenState> {
  ...
  void resetPassword(String email, String captcha) async {
    try {
      final model =
          SignUpModel(lang: 'en', email: email, gRecaptchaResponse: captcha);
      final result = await authApi.resetPassword(model);
      // currentState.context?.router.push(const AuthRoute());
      showDialogOk(currentState.context!, result, title: 'Success');
    } on Exception catch (e) {
      showDialogOk(
          currentState.context!, e.toString().replaceAll('Exception: ', ''), title: 'Error');
      // if (currentState.context?.mounted ?? false) {
      //   SnackBarService.showSnackBar(currentState.context!,
  ...
  void showDialogOk(BuildContext context, String? text, {String? title}) {
